...
const StarRow = ({ rating = 5, size = 13 }) => {
  const stars = [1,2,3,4,5].map(i => (
    <IconStar key={i} size={size} filled={i <= Math.round(rating)} color="var(--accent-line)" />
  ));
...
const TitleCard = ({ product }) => (
  <InfoCard style={{ padding: 20 }}>
    ...
    <div style={{ marginTop: 16 }}>
      ...
      <div style={{ fontSize: 12, color: "var(--text-50)", marginTop: 6 }}>
    ...
    <div style={{
      ...
      Save <span style={{ color: "var(--accent-line)", fontWeight: 500 }}>10%</span> at 5+ ·{" "}
      <span style={{ color: "var(--accent-line)", fontWeight: 500 }}>15%</span> at 10+ ·{" "}
      <span style={{ color: "var(--accent-line)", fontWeight: 500 }}>25%</span> at 25+ units
    </div>
  </InfoCard>
);
  ...
  return (
    <InfoCard>
      <div style={{
        ...
        <span className="label-up" style={{ color: "var(--text-50)" }}>Colour</span>
          ...
          return (
            <button
              ...
              style={{
                ...
                borderRadius: "50%",
                background: c.hex,
                border: selected ? "1px solid transparent" : "1px solid var(--text-25)",
...
const QuantityCard = ({ qty, setQty, sizeIdx, setSizeIdx, multi, setMulti, sizeQtys, setSizeQtys, oosSizes }) => {
  ...
  return (
    <InfoCard>
      <div className="label-up" style={{ color: "var(--text-50)" }}>How many?</div>
        ...
        {[false, true].map(mode => (
          <button
            ...
            style={{
              height: 36,
              borderRadius: 3,
              background: multi === mode ? "var(--text)" : "transparent",
              color: multi === mode ? "var(--bg)" : "var(--text-60)",
              fontSize: 13,
              fontWeight: 500,
      ...
      {!multi ? (
        ...
        <div style={{
          ...
          <div>
            <div className="label-up" style={{ color: "var(--text-50)", marginBottom: 8 }}>Size</div>
                ...
                return (
                  <button key={s} onClick={() => !oos && setSizeIdx(i)}
                    style={{
                      ...
                      color: selected ? "var(--bg)" : oos ? "var(--text-30)" : "var(--text)",
          ...
          <div>
            <div className="label-up" style={{ color: "var(--text-50)", marginBottom: 8 }}>Qty</div>
        ...
        <div style={{ marginTop: 16 }}>
          <div style={{
            ...
            {SIZES.map(s => (
              <div key={s} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                <span style={{
                  fontSize: 12, color: "var(--text-60)", fontWeight: 500
                }}>{s}</span>
          ...
          <div style={{
            ...
            <span>Total: <span style={{ color: "var(--text)" }}>{totalMulti}</span> item{totalMulti === 1 ? "" : "s"}</span>
      ...
      {!multi && oosSizes.has(SIZES[sizeIdx]) && (
        <div style={{
          ...
          try another colour or <a href="#" style={{ color: "var(--accent-line)", fontWeight: 500 }}>notify me</a>
        </div>
      )}
    </InfoCard>
  );
  ...
  return (
    <div style={{
      ...
      <button aria-label="Decrease" onClick={() => setValue(Math.max(0, value - 1))}
        style={{
          height: H, color: "var(--text)",
          borderRight: "1px solid var(--border)"
        }}>
        <IconMinus size={14} />
      </button>
      <span style={{
        textAlign: "center", fontSize: 14, fontWeight: 500, color: "var(--text)"
      }}>{value}</span>
  ...
  <div style={{
    ...
    {[
      ...
      <div key={label} style={{
        ...
        <span style={{ color: "var(--text-60)" }}>{icon}</span>
        <span>{label}</span>
      </div>
    ))}
  </div>
...
const QuickSpecsCard = ({ specs }) => (
  <InfoCard>
    <div className="label-up" style={{ color: "var(--text-50)" }}>Quick specs</div>
    <div style={{
      display: "grid",
      gridTemplateColumns: "1fr 1fr",
      columnGap: 16,
      ...
      {specs.map(([label, value]) => (
        <div key={label}>
          <div className="label-up" style={{ color: "var(--text-50)" }}>{label}</div>
...
const BulkPrompt = () => (
  <div style={{
    ...
    <a href="Quote.html" style={{ color: "var(--accent-line)", fontWeight: 500 }}>team-order service →</a>
  </div>
);
